...
export function ImageGenerator() {
  ...
  const [selectedImageModel, setSelectedImageModel] = useAtom(
    selectedImageModelAtom,
  );
  ...
  const handleGenerate = async () => {
    const provider = availableProviders.find((p) => p.capabilities?.image);
  ...
  const PromptPanel = (
    <View
      className={`h-full ${isWideScreen && generatedImage ? "w-1/2 pr-4 h-full" : "w-full"}`}
    >
  ...
  return (
    ...
    >
      ...
      {selectedImageModel && (
        <View
          className={`flex flex-1 h-full ${isWideScreen ? "flex-row" : "flex-col"}`}
        >
          {PromptPanel}
